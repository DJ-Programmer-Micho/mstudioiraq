...
const newsTable = async () => {
    const newsTableRef = db.collection('artist').doc(id).collection("01")
    .where('active', '==','1')
    .orderBy('no',"asc")
    ...
    newsTableData.docs.forEach(doc =>{
        ...
        <td class="text-left">${newsTableView.trackNameText}</td>
        <td class="text-left">${t10}</td>
        <td align-middle"><a type="button" data-toggle="modal" data-target="#exampleModalU" onclick="take('${newsTableView.trackName}')"><i class="far fa-edit"></i></a></td>            
        <td align-middle"><a onclick="del('${newsTableView.trackName}')"><i class="fas fa-trash"></i></a></td>            
        </tr>
            `
    });
